...
interface GaesteverzeichnisDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Gaesteverzeichnis['fields']) => Promise<void>;
  defaultValues?: Gaesteverzeichnis['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function GaesteverzeichnisDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: GaesteverzeichnisDialogProps) {
  const [fields, setFields] = useState<Partial<Gaesteverzeichnis['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'gaesteverzeichnis');
      await onSubmit(clean as Gaesteverzeichnis['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Gaesteverzeichnis['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Gästeverzeichnis bearbeiten' : 'Gästeverzeichnis hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="notizen_gast">Notizen</Label>
            <Textarea
              id="notizen_gast"
              value={fields.notizen_gast ?? ''}
              onChange={e => setFields(f => ({ ...f, notizen_gast: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
