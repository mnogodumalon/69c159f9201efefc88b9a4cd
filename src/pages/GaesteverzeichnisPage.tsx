...
import type { Gaesteverzeichnis } from '@/types/app';
...
export default function GaesteverzeichnisPage() {
  const [records, setRecords] = useState<Gaesteverzeichnis[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Gaesteverzeichnis | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Gaesteverzeichnis | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Gaesteverzeichnis | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getGaesteverzeichnis());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Gaesteverzeichnis['fields']) {
    await LivingAppsService.createGaesteverzeichni(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Gaesteverzeichnis['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateGaesteverzeichni(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteGaesteverzeichni(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Gästeverzeichnis im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Gästeverzeichnis suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('notizen_gast')}>
                <span className="inline-flex items-center gap-1">
                  Notizen
                  {sortKey === 'notizen_gast' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.notizen_gast ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Gästeverzeichnis. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Gästeverzeichnis löschen"
