...
2. DISPATCH ALL SUBAGENTS IN PARALLEL (in a single response):
   a) Dispatch 'dashboard_builder' to build src/pages/DashboardOverview.tsx
      ...
      - Tell it which URL params each intent supports (e.g., ?eventId=xxx&step=2) so the dashboard \
can create direct-action buttons that skip to specific wizard steps
...
in each step, what records get created/updated, what live feedback to show between steps
      - Tell it to support useSearchParams for ?step= parameter to allow deep-linking to specific steps
